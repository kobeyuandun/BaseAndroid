...
public class ZoomImageView extends ImageView
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if ((scale < SCALE_MAX && scaleFactor > SCALE_MIN) || (scale > mScale && scaleFactor < SCALE_MIN)) {
    ...
    protected boolean setFrame(int l, int t, int r, int b) {
        ...
        if (boundWidth > mCropWidth || boundHeight > mCropHeight) {
            return false;
        }
        int width = getWidth();
    ...
    public void onGlobalLayout() {
        if (isFirst) {
            ...
            float scale = 1.0f;
            if (dw < getWidth() - mOffset * 2 && dh > getHeight() - mVOffset * 2) {
                scale = (getWidth() * 1.0f - mOffset * 2) / dw;
            }

            if (dh < getHeight() - mVOffset * 2 && dw > getWidth() - mOffset * 2) {
                scale = (getHeight() * 1.0f - mVOffset * 2) / dh;
            }

            if (dw < getWidth() - mOffset * 2 && dh < getHeight() - mVOffset * 2) {
                float scaleW = (getWidth() * 1.0f - mOffset * 2) / dw;
    ...
    public Bitmap cropBitmap() {
        ...
        draw(canvas);
        return Bitmap.createBitmap(bitmap, mOffset, mVOffset, getWidth() - 2 * mOffset, getWidth() - 2 * mOffset);
    }
