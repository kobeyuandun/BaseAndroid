package com.baseandroid.repository.config.cookie;
...
public class PersistentCookieStore implements CookieStore {
    ...
    public PersistentCookieStore(Context context) {
        sharedPreferences = context.getSharedPreferences(SP_COOKIE_STORE, Context
                .MODE_PRIVATE);
    ...
    /**
     * Get the real URI from the cookie "domain" and "path" attributes, if they are not
     * set then uses the URI provided (coming from the response)
     * @param uri
     * @param cookie
     * @return
     */
    private static URI cookieUri(URI uri, HttpCookie cookie) {
        ...
        if (cookie.getDomain() != null) {
            // Remove the starting dot character of the domain, if exists (e.g: .domain
            // .com -> domain.com)
            ...
            try {
                cookieUri = new URI(uri.getScheme() == null ? "http" : uri.getScheme(),
                        domain, cookie.getPath() == null ? "/" : cookie.getPath(), null);
            } catch (URISyntaxException e) {
                Log.w(TAG, e);
            }
        }
    ...
    private void saveToPersistence(URI uri, HttpCookie cookie) {
        ...
        editor.putString(uri.toString() + SP_KEY_DELIMITER + cookie.getName(), new
   ...
   /* http://tools.ietf.org/html/rfc6265#section-5.1.3
    ...
    private boolean checkPathsMatch(String cookiePath, String requestPath) {
        return requestPath.equals(cookiePath) ||
                (requestPath.startsWith(cookiePath) && cookiePath.charAt(cookiePath
                        .length() - 1) == '/') ||
                (requestPath.startsWith(cookiePath) && requestPath.substring(cookiePath
                        .length()).charAt(0) == '/');
    }
